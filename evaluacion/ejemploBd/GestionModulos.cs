...
namespace ejemploBd
{
	public partial class GestionModulos : Form
	{
		...
		void BtnVerPreguntasClick(object sender, EventArgs e)
		{
			...
			int idModulo = Convert.ToInt32(dgvModulos.SelectedRows[0].Cells["id"].Value.ToString());
			GestionModulos fmr = new GestionModulos(id)
			
			string nombreMod = Convert.ToInt32(dgvModulos.SelectedRows[0].Cells["nombre"].Value.ToString());
